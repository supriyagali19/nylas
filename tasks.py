...
async def check_and_get_media(notetaker_id: str, meet_url: str):
    """
    Checks status, downloads media, processes it, uploads to S3, and saves all data to MongoDB.
    """
        ...
        try:
            ...
            if current_state == NotetakerState.MEDIA_AVAILABLE:
                ...
                if media_response.data.transcript and media_response.data.transcript.url:
                    # FIX: Correctly await the download and then the json parsing
                    async with httpx.AsyncClient() as http_client:
                ...
                if media_response.data.recording and media_response.data.recording.url:
                    ...
                    if "video" in content_type:
                        print("Video detected. Processing in background thread...")
                        # FIX: Run blocking CPU/IO-bound code in a separate thread
                        ...
                        print("Audio file detected. Uploading directly...")
                        await upload_file_to_s3(recording_content, f"{folder_name}/audio.mp3", "audio/mpeg")
                    
                    s3_folder_url = f"https://s3.console.aws.amazon.com/s3/buckets/{AWS_S3_BUCKET_NAME}?prefix={folder_name}/"
